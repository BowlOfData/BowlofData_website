...
--
-- Postgres is the single source of truth for newsletter content.
...
CREATE TABLE IF NOT EXISTS items (          -- articles AND papers
  ...
  category_label         text,
  FOREIGN KEY (week, year) REFERENCES weeks(week, year) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS releases (       -- AI model releases block on week pages
  id            bigserial PRIMARY KEY,
  week          int  NOT NULL,
  year          int  NOT NULL,
...
CREATE TABLE IF NOT EXISTS item_tags (      -- one row per (item, technology)
  item_id  bigint NOT NULL REFERENCES items(id) ON DELETE CASCADE,
  slug     text   NOT NULL,     -- slugify(technology)
  name     text   NOT NULL      -- canonical display name
);

CREATE INDEX IF NOT EXISTS item_tags_slug_idx ON item_tags (slug);
CREATE INDEX IF NOT EXISTS item_tags_item_idx ON item_tags (item_id);
CREATE INDEX IF NOT EXISTS items_week_idx      ON items (week, year);
